...
Ở bài [một](https://viblo.asia/p/webpack-tu-a-den-a-webpack-la-gi-1VgZveNMKAw) chúng ta đã tìm hiểu webpack là gì? Bài hôm nay sẽ học cách cài đặt webpack và thực hành trên hệ điều hành (HĐH) Win 10, vì webpack chạy trên môi trường NodeJS nên chúng ta cần cài đặt theo các bước sau:

## 1. Cài đặt NodeJS
* Tùy thuộc vào HĐH các bạn đang sử dụng mà lên trang chủ của NodeJS tải về bản tương ứng: https://nodejs.org/en/download/
* Tham khảo hướng dẫn cài đặt tại: https://vntalking.com/cai-dat-nodejs-tren-window-ubuntu.html
...
### Bước 2: Mở cmd
Click vào trong folder trên và mở `cmd` lên. Đối với HĐH Windows bạn có thể gõ cmd trên thanh địa chỉ của folder và ấn enter để mở lên

![Hình 1: Cách mở cmd trên HĐH Windows](https://images.viblo.asia/d2d53970-6058-4772-934a-c619d464cc92.png)
...
```
Tham số `--save-dev` để báo với `package.json` chỉ cài thư viện này lúc đang code phát triển dự án thôi còn khi triển khai thật thì không cần. Đó là lí do mà phần dependencies của `package.json` được chia làm 2 phần: `dependencies` và `devDependencies`
...
webpack-demo
  ...
  |- package.json
```
Mình chỉ cần quan tâm đến file `package.json` thôi và code bên trong sẽ như này, lưu ý phiên bản của `webpack` và `webpack-cli` có thể khác với của mình
```json
{
  "name": "webpack-demo",
  "version": "1.0.0",
...
* Tham khảo: https://webpack.js.org/guides/getting-started/
